...
export const registry: DocumentTypeRegistry = {
  register<T extends InputsUnion>(type: 'narrative' | 'fhir' | 'note_and_fhir', def: DocumentTypeDef<T>): void {
    defs.set(type, def as DocumentTypeDef<InputsUnion>);
  },
  get<T extends InputsUnion>(type: 'narrative' | 'fhir' | 'note_and_fhir'): DocumentTypeDef<T> | undefined {
    const d = defs.get(type);
    return d as DocumentTypeDef<T> | undefined;
  },
  all(): Array<{ type: 'narrative' | 'fhir' | 'note_and_fhir'; def: DocumentTypeDef<InputsUnion> }> {
    return Array.from(defs.entries()).map(([type, def]) => ({
      type: type as 'narrative' | 'fhir' | 'note_and_fhir',
      def,
    }));
  },
};
